...
fun WebViewScreen(streamUrl: String, onBackToSelection: () -> Unit = {}) {
    ...
    // Timer to hide the restart button after inactivity
    LaunchedEffect(showRestartButton) {
        if (showRestartButton) {
            delay(5000) // Hide button after 5 seconds of inactivity
            showRestartButton = false
        }
    }
    
    // Key event listener for the entire screen
    Box(
        modifier = Modifier
            .fillMaxSize()
            .onKeyEvent { keyEvent ->
                // Show the restart button when any key is pressed
            ...
            factory = { context ->
                WebView(context).apply {
                    ...
                    val adDomains = listOf(
                        ...
                        "advertising", "clickbank", "clicksor", "fubo.tv"  // Added fubo.tv to block list
                    )
                    ...
                    webViewClient = object : WebViewClient() {
                        ...
                        override fun shouldInterceptRequest(view: WebView?, request: WebResourceRequest?): WebResourceResponse? {
                            ...
                            // Block fubo.tv resources specifically
                            if (url.contains("fubo.tv", ignoreCase = true)) {
                                Log.d("AdBlock", "Blocked fubo.tv URL: $url")
                        ...
                        override fun shouldOverrideUrlLoading(view: WebView?, request: WebResourceRequest?): Boolean {
                            ...
                            // Block any navigation to fubo.tv
                            if (url.contains("fubo.tv", ignoreCase = true)) {
                                Log.d("AdBlock", "Blocked redirect to fubo.tv: $url")
                                return true
                            }
        ...
        // Reload button that appears on interaction
        if (showRestartButton) {
